void aBarrel()
{
  ...
  c2->SetLogx();
  TF1 *faBarrel = new TF1("faBarrel","[0]+((([1]+([2]/(x^[5])))*exp(-(x^[4]/[3]))))",1.,1000.);
  faBarrel->SetParameter(0,-1.81635);
  faBarrel->SetParameter(1,2.88284);
  faBarrel->SetParameter(2,7.12287);
  faBarrel->SetParameter(3,0.118197);
  faBarrel->SetParameter(4,-1.49896);
  faBarrel->SetParameter(5,0.5988);
...
//  legends->AddEntry(faBarrel2,"for old calib ( chi2/ndf = 9.47149 )","l");                                  
  legends->AddEntry(faBarrel, "using new calibration ","l");//(name of hist,what you want it called in legend, l=line, p=polymarker, f=boxy thing )                                                     
  legends->AddEntry(faBarrel2,"using old calibration","l");                                  
legends->Draw();
 c2->Modified();
 c2->cd();
