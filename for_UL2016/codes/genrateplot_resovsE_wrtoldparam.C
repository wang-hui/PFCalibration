#include <stdio.h>
// #include<conio.h>
void genrateplot_resovsE_wrtoldparam(string legendname, string plot_name, bool xaxis_pt)
{
...
//=========  (Mon Apr  1 19:35:54 2019) by ROOT version 6.14/06
  ...
  // sprintf(plot_name, "H_10_6_0_pre2");
  //  sprintf(plot_name, "EH_barrel_10_6_0_pre2_pt");
//  sprintf(path_root1,"/Users/bkansal/work/Ultra_legacy/final/for_UL2017/raw/response_plots_root");
//  sprintf(path_root2,"/Users/bkansal/work/Ultra_legacy/final/for_UL2017/trial1/response_plots_root/before15may");
  sprintf(path_root2,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/trial1/response_plots_root");
  sprintf(path_root1,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/for_10_0_3_calib/response_plots_root");
...
//  sprintf(hname1,"%s/%s_%s.root",path_root2,rootfile2,plot_name.c_str());
  ...
  //  sprintf(path,"../overlay_reso_plots/witholdparam/%s_%s.gif",rootfile1,plot_name);
  // sprintf(old_rel,"%s",old_release);
  // sprintf(new_rel,"%s",new_release);

  if(xaxis_pt==false){
    sprintf(hname,"%s/%s_%s_10_0_3.root",path_root1,rootfile1,plot_name.c_str());
    sprintf(hname1,"%s/%s_%s_10_6_0.root",path_root2,rootfile2,plot_name.c_str());
    sprintf(path,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/pdf/%s_%s_reso_wrtoldparam.pdf",rootfile2,plot_name.c_str());
    sprintf(path1,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/png/%s_%s_reso_wrtoldparam.png",rootfile2,plot_name.c_str());
    sprintf(path2,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/gif/%s_%s_reso_wrtoldparam.gif",rootfile2,plot_name.c_str());
  }
  else if(xaxis_pt==true){
    sprintf(hname,"%s/%s_%s_10_0_3_pt.root",path_root1,rootfile1,plot_name.c_str());
    sprintf(hname1,"%s/%s_%s_10_6_0_pt.root",path_root2,rootfile2,plot_name.c_str());
    sprintf(path,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/pdf/%s_%s_pt_reso_wrtoldparam.pdf",rootfile1,plot_name.c_str());
    sprintf(path1,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/png/%s_%s_pt_reso_wrtoldparam.png",rootfile1,plot_name.c_str());
    sprintf(path2,"/Users/bkansal/work/Ultra_legacy/final/for_UL2016/plots/overlay/overlay_wrtoldparam/responseplots/gif/%s_%s_pt_reso_wrtoldparam.gif",rootfile1,plot_name.c_str());
  }
  ...
  cout<<"file name --> "<<plot_name<<endl;
   ...
   TLegend* legends = new TLegend(0.15, 0.7, 0.9, 0.9,"","brNDC"); // the numbers determine the position of the box
      
   legends->SetFillColor(0);
   legends->SetHeader(legendname.c_str());
   legends->AddEntry(graph1,"final correction (using old parameters)","P");//(name of hist,what you want it called in legend, l=line, p=polymarker, f=boxy ting )
   legends->AddEntry(graph2,"final correction (using new parameters)","P");
   ...
   // legends->AddEntry(graph1,"for Ultralegacy 2017","P");//(name of hist,what you want it called in legend, l=line, p=polymarker, f=boxy thing )
   ...
   // legends->SetFillColor(0); 
   // legends->SetHeader("H hadrons (barrel)"); 
   // legends->AddEntry(graph1,"old calib (10_0_3)","lep");//(name of hist,what you want it called in legend, l=line, p=polymarker, f=boxy thing ) 
   // legends->AddEntry(graph2,"new calib","lep");
   // legends->Draw();
