...
void generate_1Dplots()
{
  ...
  for(int i=2; i<65;i++)
    { 
      ...
      sprintf(hname,"%s_%s_10_6_0_pre2.root",old_release,plot_name);
      sprintf(hname1,"%s_%s_10_6_0_pre2.root",new_release,plot_name);
      // sprintf(plot_name,"%s",plot);
      sprintf(hist_name,"%s%d",hist,n[i]);
      sprintf(path,"../1D_response_plots_13may/%s_%s/%s.gif",new_release,plot_name,hist_name);
   ...
   resp2->SetName("After E correction");
   //   resp2->Fit("gaus");
   resp2->Draw();
