...
//=========  (Mon Apr  1 19:35:54 2019) by ROOT version 6.14/06
  ...
  sprintf(plot_name, "Raw_H_ec_out");
  sprintf(hist, "histcorhybrid");
  double xmin= -2, xmax =1.5;
  double ymin= 0, ymax =0.11;
  int n[69];
  n[0]=0;
  n[1]=2;
  for(int i=2; i<69; i++)
  ...
  for(int i=2; i<68;i++)
    { 
      ...
      sprintf(hname,"%s_%s.root",plot_name,old_release);
      sprintf(hname1,"%s_%s.root",plot_name,new_release);
      // sprintf(plot_name,"%s",plot);
      sprintf(hist_name,"%s%d",hist,n[i]);
      sprintf(path,"../1D_response_plots/%s/%s.gif",plot_name,hist_name);
   ...
   resp2->GetYaxis()->SetRangeUser(ymin , ymax);                                                        resp2->SetName("before correction (Raw)");
   resp2->Fit("gaus");
   resp2->Draw();
